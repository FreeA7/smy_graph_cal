...
def cal_one_layer_graph_index(G_info, nodes):
    """
    计算单层图的指数
    单层图：只计算节点直接连接的权重信息，不计算间接连接的信息
    degree_centrality_normal
    degree_centrality
    degree_centrality_weight
    between_centrality
    between_centrality_normal
    closeness_centrality_nx_normal
    closeness_centrality
    closeness_centrality_smy_normal
    """
    res = defaultdict(int)

    # 基础信息
    num_of_graph_nodes = G_info['num_of_graph_nodes']
    num_of_nodes = len(nodes)

    if not num_of_graph_nodes or num_of_nodes == 1:
        return res

    # 节点信息
    centrality = G_info['centrality']
    weight_centrality = G_info['weight_centrality']
    between_centrality = G_info['between_centrality']
    between_centrality_normal = G_info['between_centrality_normal']
    closeness_centrality = G_info['closeness_centrality']
...
def cal_one_layer_graph_index_by_node(primary_key, graph_info, nodes):
    """
    计算单层图的指数
    单层图：只计算节点直接连接的权重信息，不计算间接连接的信息
    degree_centrality_normal
    ...
    # 基础信息
    num_of_graph_nodes = graph_info[primary_key]['num_of_graph_nodes']
    num_of_nodes = len(nodes)
    ...
    # 节点信息
    centrality = graph_info[primary_key]['centrality']
    weight_centrality = graph_info[primary_key]['weight_centrality']
    between_centrality = graph_info[primary_key]['between_centrality']
    between_centrality_normal = graph_info[primary_key]['between_centrality_normal']
    closeness_centrality = graph_info[primary_key]['closeness_centrality']

    for node in nodes:
        node_info = defaultdict(int)
        node_info['node'] = node
        # 目标节点指标计算
        node_info['degree_centrality_normal'] = centrality.get(node)
        node_info['degree_centrality'] = centrality.get(node)
        node_info['degree_centrality_weight'] = weight_centrality[node]
        node_info['between_centrality'] = between_centrality.get(node)
        node_info['between_centrality_normal'] = between_centrality_normal.get(node)
        node_info['closeness_centrality_nx_normal'] = closeness_centrality.get(node)
        if node_info['closeness_centrality_nx_normal']:
            node_info['closeness_centrality'] = (num_of_graph_nodes - 1) / node_info['closeness_centrality_nx_normal']
            node_info['closeness_centrality_smy_normal'] = 1 / node_info['closeness_centrality_nx_normal']
        else:
            node_info['closeness_centrality'] = node_info['closeness_centrality_smy_normal'] = 0
        res.append(node_info)

    return res


def cal_graph_index(G):
    """
    clustering_without_zero
    ...
    res['centrality'] = nx.degree_centrality(G)
    res['weight_centrality'] = G.degree(weight='weight')
    res['between_centrality'] = nx.betweenness_centrality(G, normalized=False)
    res['between_centrality_normal'] = nx.betweenness_centrality(G)
    res['closeness_centrality'] = nx.closeness_centrality(G)

    res['num_of_graph_nodes'] = len(G.nodes)
